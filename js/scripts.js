...
function contar() {
    ...
    selectedServices.forEach(service => {
        total += service.price;
        servicesText += `<p>${service.name} - <strong>$ ${parseFloat(service.price).toLocaleString('es-AR')}</strong></p>`;
    });
...
function generarPresupuesto() {
    ...
    selectedServices.forEach(service => {
        total += service.price;
        servicesText += `${service.name} - $ ${parseFloat(service.price).toLocaleString('es-AR')}\n`;
    });
    ...
    selectedServices.forEach(service => {
        const maxWidth = 130;
        const splitServiceName = doc.splitTextToSize(service.name, maxWidth);
        ...
        doc.text(`$ ${parseFloat(service.price).toLocaleString('es-AR')}`, 190, yOffset - 7, { align: 'right' });
